...
class stPMH(object):
    ...
    def rejectParameters(self, thSys,):
        if ((self.PMHtype == "qPMH2") & (self.iter > self.memoryLength)):
            ...
            self.prior[self.iter, :] = self.prior[self.iter -
                                                  self.memoryLength, :]
            self.gradient[self.iter,
                          :] = self.gradient[self.iter - self.memoryLength, :];
            self.hessian[self.iter, :,
                         :] = self.hessian[self.iter - self.memoryLength, :, :];
    ...
    def adaptHessian(self):
        self.invHessian = np.cov(self.th[range(
            self.nBurnIn - int(self.adaptHessianNoSamples), self.nBurnIn), :].transpose());
        print('pmh: adapted Hessian using the last ' + str(self.adaptHessianNoSamples) +
              ' samples of the chain during burn-in with diagonal ' + str(np.round(np.diag(self.invHessian), 3)) + ".");
    ...
    def writeToFile(self, sm=None, fileOutName=None):
        ...
        if (self.PMHtype == "PMH1"):
            ...
            for kk in range(0, self.nIter):
                ngrad[kk, :] = np.dot(
                    np.sign(self.gradient[kk, :]), self.invHessian);
        elif (self.PMHtype == "PMH2"):
            for kk in range(0, self.nIter):
                ngrad[kk, :] = np.dot(
                    self.gradient[kk, :], np.linalg.pinv(self.hessian[kk, :, :]));

        # Construct the columns labels
        columnlabels = [None] * (3 * self.nPars + 3)
        for ii in xrange(3 * self.nPars + 3):
            columnlabels[ii] = ii
        ...
        out = np.hstack((self.th, self.thp, ngrad,
                         self.aprob, self.ll, self.accept))
                ...
                if hasattr(sm, 'filterType'):
                    if (sm.filterType == "kf"):
                        fileOutName = 'results/' + str(self.filePrefix) + '/' + str(
                            self.PMHtype) + '_' + str(sm.filterType) + '/' + str(self.dataset) + '.csv'
                    else:
                        fileOutName = 'results/' + str(self.filePrefix) + '/' + str(self.PMHtype) + '_' + str(
                            sm.filterType) + '_N' + str(sm.nPart) + '/' + str(self.dataset) + '.csv'
                else:
                    # Fallback
                    fileOutName = 'results/' + \
                        str(self.filePrefix) + '/' + str(self.PMHtype) + \
                        '/' + str(self.dataset) + '.csv'

            elif hasattr(sm, 'smootherType'):
                # Compile a file name for PMH algortihms using a smoother
                if (sm.smootherType == "rts"):
                    fileOutName = 'results/' + str(self.filePrefix) + '/' + str(self.PMHtype) + '_' + str(
                        sm.filterType) + '_' + str(sm.smootherType) + '/' + str(self.dataset) + '.csv'
                else:
                    fileOutName = 'results/' + str(self.filePrefix) + '/' + str(self.PMHtype) + '_' + str(
                        sm.filterType) + '_' + str(sm.smootherType) + '_N' + str(sm.nPart) + '/' + str(self.dataset) + '.csv'
            else:
                # Fallback
                fileOutName = 'results/' + \
                    str(self.filePrefix) + '/' + str(self.PMHtype) + \
