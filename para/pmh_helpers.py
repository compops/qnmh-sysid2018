...
def setSettings(pmh, vers):
    ...
    if (pmh.nIter == None):
        pmh.nIter = 1000
        print("pmh (nIter): no number of iterations given, defaulting to " +
              str(pmh.nIter) + ".");

    if (pmh.nBurnIn == None):
        pmh.nBurnIn = 250
        print("pmh (nBurnIn): no length of burn-in given, defaulting to " +
              str(pmh.nBurnIn) + ".");

    if (pmh.nProgressReport == None):
        pmh.nProgressReport = 100
        print("pmh (nProgressReport): period for progress reports given, defaulting to " +
              str(pmh.nProgressReport) + ".");

    if (pmh.dataset == None):
        pmh.dataset = 0
        print("pmh (dataset): no number of data set given, defaulting to " +
              str(pmh.dataset) + ".");

    if (pmh.filePrefix == None):
        pmh.filePrefix = "model"
        print("pmh (filePrefix): no short name for model given, defaulting to " +
              str(pmh.filePrefix) + ".");
    ...
    if ((pmh.adaptHessianNoSamples == None) & (pmh.adaptHessianAfterBurnIn == True)):
        ...
        print("pmh (adaptHessianNoSamples): defaulting to using half the burn-in, i.e. " +
              str(pmh.adaptHessianNoSamples) + " samples to adapt the Hessian.");
    ...
    if ((pmh.writeOutProgressToFileInterval == None) & (pmh.writeOutProgressToFile == None)):
        ...
        print("pmh (writeOutProgressToFileInterval,writeOutProgressToFile): defaulting write out current progress to file for each: " +
              str(pmh.writeOutProgressToFileInterval) + " step of the algorithm.");
    ...
    if ((vers == "PMH0") | (vers == "PMH1")):
        ...
        if (pmh.stepSize == None):
            pmh.stepSize = 0.10
            print("pmh (stepSize): no step size given, defaulting to " +
                  str(pmh.stepSize) + " for all parameters.");
    ...
    if ((vers == "pPMH1") | (vers == "pPMH0") | (vers == "bpPMH1")):
        ...
        if ((vers == "pPMH0") & (pmh.stepSize == None)):
            ...
            print("pmh0 (stepSize): no step size given, defaulting to optimal " +
                  str(pmh.stepSize) + " for all parameters.");

        if ((vers == "pPMH1") & (pmh.stepSize == None)):
            pmh.stepSize = 1.125 / np.sqrt(pmh.nPars**(1.0 / 3.0))
            print("pmh1 (stepSize): no step size given, defaulting to optimal " +
                  str(pmh.stepSize) + " for all parameters.");

        if (pmh.invHessian == None):
            print(
                "pmh (invHessian): no inverse Hessian given, defaulting to unit matrix.");
    ...
    if ((vers == "PMH2") | (vers == "qPMH2")):
        ...
        if (pmh.makeHessianDiagonal == None):
            ...
            print(
                "pmh2 (makeHessianDiagonal): defaulting to use full Hessian (not only a diagonal Hessian).");

        # Fix for old run scripts
        if (pmh.makeHessianPSDreject == True):
            print(
                "pmh2: using makeHessianPSDreject, change to pmh.makeHessianPSDmethod = reject.")
            pmh.makeHessianPSDmethod = "reject"

        # Fix for old run scripts
        if (pmh.makeHessianPSDregularise == True):
            print(
                "pmh2: using makeHessianPSDregularise, change to pmh.makeHessianPSDmethod = regularise.")
            pmh.makeHessianPSDmethod = "regularise"

        # Fix for old run scripts
        if (pmh.makeHessianPSDhybrid == True):
            print(
                "pmh2: using makeHessianPSDhybrid, change to pmh.makeHessianPSDmethod = hybrid.")
            pmh.makeHessianPSDmethod = "hybrid"

        if (pmh.makeHessianPSDmethod == None):
            pmh.makeHessianPSDmethod = "regularise"
            print(
                "pmh2 (makeHessianPSDmethod): defaulting to make the Hessian PSD by regularisation.");

        if ((pmh.makeHessianPSDmethod == "hybrid") & (pmh.PSDmethodhybridSamps == None)):
            pmh.PSDmethodhybridSamps = 1500
            print("pmh (PSDmethodhybridSamps): hybrid method selected by no sample to use for covariance estimation not given, defaulting to " +
                  str(pmh.makeHessianPSDhybridNoSamples) + ".");
    ...
    if (vers == "qPMH2"):
        ...
        if (pmh.qPMHadaptInitialHessian == None):
            ...
            print(
                "qpmh2 (qPMHadaptInitialHessian): defaulting to not adapting initial Hessian.");

        if (pmh.memoryLength == None):
            pmh.memoryLength = 20
            print("qpmh2 (memoryLength): defaulting to use memory length " +
                  str(pmh.memoryLength) + " to estimate the Hessian.");

        if (pmh.epsilon == None):
            pmh.epsilon = 400
            print("qpmh (epsilon): defaulting to use a diagonal matrix with" +
                  str(pmh.epsilon) + " on the diagonal as initial Hessian.");
    ...
    if (pmh.proposeRVs == True):
        ...
        if (pmh.rvnSamples == None):
            ...
            print("cpmh (rvnSamples): defaulting to have " +
                  str(pmh.rvnSamples) + " random variables in the Markov chain.");

        if (pmh.alpha == None):
            pmh.alpha = 0.0
            print("cpmh (alpha): defaulting to have " +
                  str(pmh.rvnSamples) + " as probability for global move.");

        if (pmh.sigmaU == None):
            pmh.sigmaU = 0.80
            print("pmh (sigmaU): defaulting to have " +
                  str(pmh.sigmaU) + " as scaling in local random walk move.");
        ...
        if ((pmh.adaptHessianRecursively == True) & (pmh.adaptHessianRecursivelyIterLimitFrom == None)):
            pmh.adaptHessianRecursivelyIterLimitFrom = np.floor(
                pmh.nBurnIn * 0.50)
        ...
        if ((pmh.adaptHessianRecursively == True) & (pmh.adaptHessianRecursivelyIterLimitTo == None)):
            ...
            print(
                "pmh (adaptHessianRecursivelyIterLimit): defaulting to adapt Hessian up tp burn-in.");
...
def progressPrint(pmh):
    print("################################################################################################ ")
    print(" Iteration: " + str(pmh.iter + 1) +
          " of : " + str(pmh.nIter) + " completed.")
    print("")
    ...
    if ((pmh.PMHtype == "qPMH2") & (pmh.iter > pmh.memoryLength)):
        ...
        print("%.4f" %
              np.mean(pmh.nHessianSamples[range(pmh.memoryLength, pmh.iter)]))
    print("################################################################################################ ")
...
def calcSJD_prototype(pmh):
    ...
    MCtrace = pmh.th[range(int(pmh.nBurnIn), int(pmh.iter)), :];
    SJD = np.sum(np.linalg.norm(np.diff(MCtrace, axis=0),
                                2, axis=1)**2) / (MCtrace.shape[0] - 1.0)
...
def logdmvt(x, mu, Sigma, df):
    ...
    part4 = - 0.5 * (df + p) * np.log(1.0 + np.dot(np.dot((x - mu),
                                                          np.linalg.inv(Sigma.astype(float))), (x - mu)) / df)
