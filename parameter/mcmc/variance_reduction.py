...
def zero_variance_linear(mho):
    """Implements zero-variance post processing with linear correction."""
    no_params_to_estimate = mho.settings['no_params_to_estimate']
    no_iters = mho.settings['no_iters']
    no_burnin_iters = mho.settings['no_burnin_iters']

    a_hat = np.zeros((no_params_to_estimate, no_params_to_estimate))
    idx = range(no_burnin_iters, no_iters)
    gradients = -0.5 * mho.gradient[idx, :]
    for i in range(no_params_to_estimate):
        params = mho.free_params[idx, i]
    ...
    idx = range(no_burnin_iters, no_iters)
    corrected_params = mho.free_params[idx, :]
