...
class KalmanMethods(object):
    ...
    def filter(self, model):
        ...
        self.filt_state_est = np.zeros((model.no_obs + 1))
        self.filt_state_cov = np.zeros((model.no_obs + 1))
        self.kalman_gain = np.zeros(model.no_obs + 1)
        self.log_like = 0.0
        ...
        for i in range(1, model.no_obs + 1):
            # Prediction step
            self.pred_state_est[i] = mu
            self.pred_state_est[i] += phi * (self.filt_state_est[i-1] - mu)
    ...
    def smoother(self, model):
        ...
        self.smo_state_est = np.zeros((model.no_obs + 1, 1))
        self.smo_state_cov = np.zeros((model.no_obs + 1, 1))
